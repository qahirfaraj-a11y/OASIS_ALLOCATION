...
logger = logging.getLogger("DebugMicro")

def debug_micro_sim():
    budget_override = 3570000.0 # 3.57M
    target_month = "JAN"
    duration_days = 12
    benchmark_value = 115_000_000.0
    
    logger.info(f"--- STARTING DEBUG MICRO SIM (Budget: ${budget_override:,.0f}) ---")
    ...
    # Initial Stock Check
    milk_items = [r for r in allocated_items if "MILK" in str(r['product_name']).upper() and "FRESH" in str(r['product_category']).upper()]
    logger.info(f"Allocated {len(milk_items)} Fresh Milk SKUs.")
    for m in milk_items[:5]:
        logger.info(f"  > {m['product_name']}: Qty={m['recommended_quantity']} (Sales/Day={m['avg_daily_sales']:.1f}) -> Days={m['recommended_quantity']/m['avg_daily_sales']:.1f}")
    ...
    for day in range(1, duration_days + 1):
        ...
        if received > 0:
            logger.info(f"Day {day}: Received {received} SKUs.")
        
        # Check Stock BEFORE Sales (Morning Status)
        # Trace Brookside
        for sku, data in tracker.inventory.items():
            if "BROOKSIDE 500ML DAIRY BEST" in sku:
                logger.info(f"Day {day} AM: {sku} | Stock={data['current_stock']:.1f}")

        # Run Sales
        daily_stats = tracker.process_daily_sales(sales_sim, day, month_factor=month_factor, store_scale_factor=1.0)
        
        # Check Stock AFTER Sales (Evening Status) / BEFORE Reorder
        for sku, data in tracker.inventory.items():
            if "BROOKSIDE 500ML DAIRY BEST" in sku:
                logger.info(f"Day {day} PM: {sku} | Stock={data['current_stock']:.1f} | Lost={data['lost_sales_units']:.1f}")
        ...
        if draft_orders:
            for do in draft_orders:
                if "BROOKSIDE 500ML DAIRY BEST" in do['sku']:
                    logger.info(f"  Day {day} ORDER PLACED: {do['sku']} Qty={do['qty']}")
                
                # Assume 2 day lead time for Fresh? 
                # Need to check what Replenisher assumes or Allocation assumes.
                arrival = day + 2 # Order Day 1 -> Arrive Day 3?
