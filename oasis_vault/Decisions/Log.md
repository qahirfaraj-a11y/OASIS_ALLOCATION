...
## [2026-08-25] The ordering formula derived; ST-GAT assessed; an allocation fix reverted
- **Problem**: Replenishment shipped as an Odoo module, but its numbers could not be defended — the engine ordered ~2x the interval each delivery had to serve, and every attempt to check the coverage horizon compared one estimate of supplier cadence against another, which is circular.
- **Solution**: Derived the order quantity from the periodic-review problem it actually is — `S = d(R+L) + z·sqrt((R+L)σd² + d²σL²)` — and mapped the code onto it. Three defects, ranked by measured worth: R uses the OBSERVED order gap rather than a policy review period (2.14x on working capital), σ_L is absent entirely though it is the larger variance term (2.22d spread against a 2.29d mean), and z is a product of five multipliers and eleven constants worth only 1.52x across the whole service range. Built `--mode residual-cover`, the one check that cannot be circular because it is taken after the fact. Built the derived model behind `OASIS_ORDER_MODEL`, default off.
- **The finding**: `supplier_weekly_schedule.json` declares an order weekday for **940 suppliers** — R was in the client's own data, unused for months, while the engine inferred it from history contaminated by past ordering. Separately, cadence is not a point estimate at all: gap CV median 1.27, only 4% of lines have a rhythm worth the name. And R is a **commitment, not a parameter** — the derived model lands on 0.97x cover for 41% of the capital but runs 49 lines dry, because it sizes for a 7-day review while deliveries arrive every 15.
- **Also**: proved the ST-GAT site-selection chain end to end on four real Nairobi sites (location → Huff score → store type → capital → tier → basket). The Huff gravity model and cannibalisation analysis are sound; **both ML artefacts are trained on synthetic labels from hand-written formulas with no validation split**, and the "GAT" is a GCN with no attention. The ordering gate holds — the unvalidated model moves no POs.
- **Outcome**: Ten commits. Also seeded the depot from a year of the client's real PO/GRN books (7,723 orders, 9,297 receipts, stock untouched), which immediately exposed that the rhythm derivation wrote lower-case supplier keys every consumer looks up in upper case — zero of 486 matched. **One change was reverted**: rebuilding department weights from turnover share halved the documented "Staples 60%" split and inverted Day-1 survival priority. The error was evaluating an INITIAL-LOAD allocator with replenishment concepts. See [[Ordering_Formula_And_Site_Selection_2026-08]].
