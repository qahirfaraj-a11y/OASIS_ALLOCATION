...
logger = logging.getLogger("DebugNano")

def debug_nano_sim():
    budget_override = 170000.0 # 170k
    target_month = "FEB" 
    duration_days = 10
    benchmark_value = 115_000_000.0
    
    logger.info(f"--- STARTING DEBUG NANO 170k SIM (Budget: ${budget_override:,.0f}) ---")
    ...
    # Why is Day 1 Fill Rate 47%? Means we missed ~53% of demand.
    # What items are being demanded but not stocked?
    # We can't know exactly what the Simulator will demand without running it, 
    # but we can guess it's High Velocity items.
    
    top_velocity_items = sorted(recommendations_in, key=lambda x: x['avg_daily_sales'], reverse=True)[:20]
    logger.info("--- Top 20 High Velocity Candidates Check ---")
    for cand in top_velocity_items:
         # Find in allocation
         found = next((x for x in allocated_items if x['product_name'] == cand['product_name']), None)
         if found:
              days_cover = found['recommended_quantity'] / (found['avg_daily_sales'] * month_factor)
              logger.info(f"[OK] {cand['product_name'][:30]}: Alloc={found['recommended_quantity']} (Cov={days_cover:.1f} days)")
         else:
              logger.warning(f"[MISSING] {cand['product_name'][:30]}: AvgSales={cand['avg_daily_sales']:.2f}")
