"""
Mock POS sales simulator (transactional).

A standalone "POS on the side" that streams **customer sales** into the POS/ERP
database while the consoles run on historical data — so we can watch how OASIS
*reacts* to new transactional data through its normal views (stock depletes →
at-risk → ordering recommends replenishment).

**Purely transactional.** It rings up bills (POS_SALES_HDR/DTL) and decrements
on-hand stock as a till does — and does NOT restock. Restocking/GRN is a
procurement flow OASIS recommends, not something a POS emits.

    python entrypoint.py --mode pos-inject --batches 20 --interval 8

This mutates the target DB (POS_SALES + STOCK_MASTER). Point OASIS_DB_PATH at a
copy if you want to preserve a baseline.
"""
...
def build_bill(org: str, bill_no: str, bill_dt: str, lines: List[SaleLine]) -> tuple:
    ...
    hdr = {
        ...
        "CUST_CD": "WALKIN", "COUNTER_CD": "POS", "LEVEL_NUMBER": 1,
        "TOTAL_QTY": total_qty, "TOTAL_AMT": net, "NET_AMT": net,
        "TAX_AMT": 0.0, "DISC_AMT": 0.0, "PAYMENT_MODE": "CASH",
        "VOID_FLAG": "F", "CUS_REF_CODE": "", "CUS_REF_REMARKS": "MOCK_POS",
    }
    dtl = []
...
def inject_once(db_path: str, org: Optional[str] = None, sku_count: int = 8,
                max_qty: int = 5, deplete: bool = True,
                seed: Optional[int] = None) -> dict:
    """Ring up one customer bill and decrement on-hand stock (transactional).

    No restocking — a till only sells. Returns a summary of the sale.
    """
    ...
    try:
        ...
        for itm, name, stock, price in picks:
            ...
            if deplete and rng.random() < 0.35:
                qty = stock                      # a customer clears the shelf
            else:
        ...
        bill_no = f"POS{datetime.now().strftime('%H%M%S%f')[:11]}"
        ...
        return {"org": org, "bill_no": bill_no, "lines": len(lines),
                "units_sold": sum(line.qty for line in lines), "sold": sold}
    finally:
        conn.close()
...
def run_injector(db_path: str, batches: int = 10, interval: float = 15.0,
    ...
    """Stream `batches` customer bills, `interval` seconds apart (integration)."""
    import time
    print(f"[mock-pos] target={db_path} batches={batches} interval={interval}s (sales only)")
    for b in range(1, batches + 1):
        r = inject_once(db_path, org=org, sku_count=sku_count)
        if r.get("error"):
            print(f"[{b}/{batches}] {r['org']}: {r['error']}")
        else:
            depleted = sum(1 for s in r["sold"] if s["new_stock"] <= 0)
            print(f"[{b}/{batches}] {r['org']} bill {r['bill_no']}: "
                  f"{r['lines']} lines, {r['units_sold']:.0f} units sold, "
                  f"{depleted} SKU(s) cleared")
        if b < batches:
