...
# --- Configuration ---
# Resolved from the install, never hardcoded: these were absolute paths on one
# developer's machine, so the module could only ever run there. OASIS_DATA_DIR
# overrides for containers and tests.
_PKG_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
DATA_DIR = os.getenv("OASIS_DATA_DIR", os.path.join(_PKG_ROOT, "oasis", "data"))

#: Optional allocation scorecard. NOT shipped and NOT required: it holds one
#: retailer's per-SKU revenue, margins and named supplier terms, which must
#: never land in another client's install. When absent the simulator runs from
#: the store's own enriched products (pre_enriched_products), which is the
#: correct source for a client anyway. OASIS_SCORECARD_FILE points at one
#: explicitly where an operator has their own.
SCORECARD_FILE = os.getenv(
    "OASIS_SCORECARD_FILE",
    os.path.join(_PKG_ROOT, "Full_Product_Allocation_Scorecard_v7.csv"))
