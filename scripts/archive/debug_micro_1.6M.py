...
logger = logging.getLogger("Debug1.6M")

def debug_16m_sim():
    budget_override = 1600000.0 # 1.6M
    target_month = "FEB" 
    duration_days = 12
    benchmark_value = 115_000_000.0
    
    logger.info(f"--- STARTING DEBUG 1.6M SIM (Budget: ${budget_override:,.0f}) ---")
    ...
    # Check Pruning Log?
    # We can't see the log unless we run it, but we can infer from Days Cover of Milk
    # High Velocity Check
    milk_items = [r for r in allocated_items if "FRESH MILK" in str(r['product_category']).upper()]
    # Sort by sales
    milk_items.sort(key=lambda x: x['avg_daily_sales'], reverse=True)
    
    if milk_items:
        top_milk = milk_items[0]
        days_cover = top_milk['recommended_quantity'] / (top_milk['avg_daily_sales'] * month_factor)
        logger.info(f"Top Milk: {top_milk['product_name']} | Qty={top_milk['recommended_quantity']} | Sales={top_milk['avg_daily_sales']*month_factor:.1f} | Cover={days_cover:.1f} days")
    else:
        logger.warning("NO MILK ALLOCATED!")
