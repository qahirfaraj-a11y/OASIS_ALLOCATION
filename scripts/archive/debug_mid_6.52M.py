...
logger = logging.getLogger("Debug6.52M")

def debug_6p52m_sim():
    budget_override = 6520000.0 # 6.52M
    target_month = "FEB" 
    duration_days = 12
    benchmark_value = 115_000_000.0
    
    logger.info(f"--- STARTING DEBUG 6.52M SIM (Budget: ${budget_override:,.0f}) ---")
    ...
    for day in range(1, duration_days + 1):
        ...
        if draft_orders:
            for do in draft_orders:
                # DYNAMIC LEAD TIME LOGIC (Fix from prev step)
