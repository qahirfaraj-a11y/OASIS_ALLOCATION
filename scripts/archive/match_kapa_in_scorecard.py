...
def check():
    ...
    df_scorecard['name_upper'] = df_scorecard['name'].astype(str).str.strip().str.upper()
    
    print("=== Matching catalog items in active_sku_scorecards.csv ===")
    matches = []
    for s in master_skus:
        m = df_scorecard[df_scorecard['name_upper'] == s]
        if m.empty:
            m = df_scorecard[df_scorecard['name_upper'].str.contains(s, regex=False)]
        if not m.empty:
            row = m.iloc[0]
            matches.append({
                'catalog_name': s,
